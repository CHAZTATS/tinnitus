...
## Native Audio Android patch persistence

This project includes a local `patch-package` patch for `@capacitor-community/native-audio@8.0.0` to preserve therapy playback when the app is backgrounded.

What it changes:

- Patches Android plugin lifecycle behavior in `NativeAudio.java`.
- Prevents automatic pause of all audio assets during `handleOnPause`.

Where it lives:

- `patches/@capacitor-community+native-audio+8.0.0.patch`

How it is applied:

- `package.json` includes `postinstall: patch-package`
- Running `npm install` automatically re-applies this patch.
...
1. The upstream plugin releases a version that includes a permanent Android background lifecycle fix.
2. You upgrade `@capacitor-community/native-audio` to that fixed version.
3. Background playback is validated on real Android devices without this patch.
4. Then remove:
   - `patches/@capacitor-community+native-audio+8.0.0.patch`
   - `postinstall` script and `patch-package` dev dependency (if no longer needed elsewhere).
